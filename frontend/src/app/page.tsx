      ...
      <div className="max-w-4xl mx-auto px-6 py-10">
        ...
        <div className="mb-8">
          ...
          <p className="text-stone-400 text-sm mt-1">
            Here's what's happening across your inbox and relationships
          </p>
        </div>
          ...
          {[
            {
              ...
              sub: `${analytics?.emails_by_category?.important || 0} important`,
            },
            {
              label: "Contacts",
              value: analytics?.total_contacts?.toLocaleString(),
              sub: "relationship scored",
            },
        ...
        <div className="bg-white rounded-xl border border-stone-100 p-4 mb-8">
          <div className="flex gap-3 items-center">
            <div className="w-8 h-8 rounded-full bg-teal-50 flex items-center justify-center text-teal-600 text-sm flex-shrink-0">
              ✦
            </div>
            <input
                ...
                <div key={e.gmail_id} className="flex items-start gap-3">
                  ...
                  <span
                    className={`text-xs px-2 py-0.5 rounded-full flex-shrink-0 ${
                      badgeColors[e.category] || badgeColors.unknown
                    }`}
                  >
                    {e.category}
                  </span>
                  ...
                  <div className="text-right w-12 flex-shrink-0">
                    <div className="h-1 bg-stone-100 rounded-full overflow-hidden">
                      <div
                        ...
                        style={{
                          width: `${Math.min(c.relationship_score, 100)}%`,
                        }}
                      />
                    </div>
                    <p className="text-xs text-stone-300 mt-1">
                      {Math.round(c.relationship_score)}
                    </p>
                  </div>
                </div>
              ))}
        ...
        <div className="bg-white rounded-xl border border-stone-100 p-4">
          ...
          <div className="space-y-3">
            {Object.entries(analytics?.emails_by_category || {}).map(
              ([cat, count]: any) => (
                <div key={cat}>
                  ...
                  <div className="h-1.5 bg-stone-100 rounded-full overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${Math.round(
                          (count / analytics.total_emails) * 100
                        )}%`,
                        background:
                          cat === "important"
                            ? "#2dd4bf"
